  ...
  render: () => (
    ...
    >
      ...
      <div>
        ...
        <Input variant="secondary" placeholder="Secondary input (orange focus)" />
      </div>
      <div>
        <label
          style={{
...
export const ValidationDemo: Story = {
  render: () => {
    ...
    return (
      ...
      >
        <h3 style={{ margin: 0, fontFamily: "IBM Plex Sans JP" }}>Form Validation Demo</h3>
          ...
          >
            {nickname && !nicknameValid && (
              <p style={{ fontSize: "0.8rem", color: "#dc2626" }}>
                {nickname.length < 2 ? "Too short (min 2 chars)" : "Too long (max 20 chars)"}
              </p>
            )}
            {nickname && nicknameValid && (
              <p style={{ fontSize: "0.8rem", color: "#10b981" }}>✓ Nickname is valid</p>
            )}
            <span style={{ fontSize: "0.8rem", color: "#6b7280" }}>{nickname.length}/20</span>
          </div>
        </div>
      </div>
    );
...
export const AllInputsShowcase: Story = {
  render: () => (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        <Input type="text" placeholder="お好きな名前を入力してください" maxLength={20} />
        <Input type="email" placeholder="example@email.com" variant="primary" />
        <Input type="password" placeholder="パスワードを入力" variant="primary" />
        <Input
          type="text"
          placeholder="祝福メッセージを入力してください..."
          variant="accent"
